...
onSnapshot(collection(db, "jobseekerRequests"), (snapshot) => {
    tableBody.innerHTML = "";
...
window.approveRequest = async function(docId, email, name, status) {
    ...
    try {
        const requestRef = doc(db, "jobseekerRequests", docId);
        await updateDoc(requestRef, { status: "Approved" });

        emailjs.send("service_k0jg0op", "template_2vmn8ch", {
            name: name,
            email: email,
            status: "APPROVED"
...
window.deleteRequest = async function(docId) {
    if(confirm("Are you sure you want to delete this request permanently?")) {
        try {
            await deleteDoc(doc(db, "jobseekerRequests", docId));
        } catch (error) {
